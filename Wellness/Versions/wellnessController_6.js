...
wellApp.directive('wellDir', function(){
	return{
		...
		template:'<div id="{{activity.id}}" class="col-sm-4 divStyle">' +
				  '<a href="#/{{activity.pageLink}}" class="noDec"><img style="position:relative;" data-original="{{activity.image}}" class="lazy imgLazy" ng-src="GroupImages/Misc/loading.gif"/>'+
				  '<h4 class="h4Style">{{activity.title}}</h4><hr />'+
				  '<p class="paraStyle"><font color="#000000">{{activity.desc}}</font></p>'+
				 '</a>' +
				 '<div style="position:absolute; height:50px; width:368px; margin-left:-15px; margin-bottom:10px; margin-top:-132px;">'+
