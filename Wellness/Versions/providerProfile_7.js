...
providerProfile.directive('certDir', function(){
	return{
		...
		replace:true,
		link: function(scope, element, attrs){
			$("#addCert1").unbind("click");
			$("#addCert1").click(function(e){
				scope.certCount = scope.certCount + 1 ; console.log(scope.certCount);
				e.preventDefault();
				var newCert = '<form name="formCert'+scope.certCount+'" id="formCert'+scope.certCount+'" novalidate class="well" style="width:820px; height:auto;"'; 
				newCert += 'ng-hide="hideCertForm" ng-submit="certSubmit()" role="form" cancel-dir>';
				newCert += '<div class="form-group" style="width:500px;">';
				newCert += '<label class="control-label" for="certName">Certification Name</label>';
				newCert += '<input id="certName" name="certName" ng-model="cert.certName" required type="text" class="form-control"/>';
				newCert += '</div>';
				newCert += '</form>';
								
				
				$("#formCert1").prepend(newCert);
